...
# Define the presets for biome thresholds
presets = {
    "Aride": {
        "temperature_thresholds": [-0.1, -0.05, 0.05, 0.1, 0.25],
        "precipitation_thresholds": [-0.6, 0.6],
    },
    "Normal": {
        "temperature_thresholds": [-0.1, -0.05, 0.05, 0.1, 0.25],
        "precipitation_thresholds":  [-0.2, -0.15, -0.1, 0.1, 0.25],
    },
    "Humide": {
        "temperature_thresholds": [-0.1, -0.05, 0.05, 0.1, 0.25] ,
        "precipitation_thresholds": [-0.1, 0.1],
    }
}
...
def show_map_generator():
    def on_generate_map(tab_personalisation):
        if root.winfo_exists():  # Check if the window still exists
            ...
            tab_personalisation[3] = presets[climate_combobox.get()]  # Update the preset value
    ...
    climate_combobox = ttk.Combobox(root, values=["Aride", "Normal", "Humide"])
    ...
    # Initialize a list to hold scale, octaves, climatX, and preset values
    tab_personalisation = [scale_entry.get(), octaves_entry.get(), None, None]

    root.mainloop()
    # Return the list containing updated scale, octaves, climatX, and preset values
...
def assign_biomes(temperature_map, precipitation_map, temperature_thresholds, precipitation_thresholds):
    ...
    temperature_thresholds = preset["temperature_thresholds"]
    precipitation_thresholds = preset["precipitation_thresholds"]
...
# Fontion pour finalement generer la carte de biomes et de méteo
def generate_world(seed, WORLD_SIZE_X, WORLD_SIZE_Y, scale, octaves, preset):
    ...
    temperature_thresholds = preset["temperature_thresholds"]  # Retrieve temperature thresholds from the preset
    precipitation_thresholds = preset["precipitation_thresholds"]  # Retrieve precipitation thresholds from the preset
    ...
    biomes = assign_biomes(temperature_map, precipitation_map, temperature_thresholds, precipitation_thresholds)  # Pass all required arguments
...
scale,octaves,climatX, preset = show_map_generator() # Echelle dans la génération du bruit de Perlin
...
biomes, rain_intensity, snow_intensity = generate_world(seed, WORLD_SIZE_X, WORLD_SIZE_Y, scale, octaves, preset)
