...
from tkinter import simpledialog # La boite de dialog pour faire entrer les valeurs
    ...
    for i in range(world_size_x):
        for j in range(world_size_y):
            world[i][j] = noise.pnoise2(i / scale, j / scale, octaves=octaves, persistence=persistence,
                                        lacunarity=lacunarity, repeatx=1024, repeaty=1024, base=hash(seed) % 1024)
    return world
...
def get_seed():
    root = tk.Tk()
    root.withdraw()
    seed = simpledialog.askstring("Entrez votre Seed", "Faites entrer une chaîne de LETTRES et/ou de CHIFFRES pour générer votre carte. \n(Plus la chaîne est courte, meilleure sera la carte. Les chiffres marchent mieux car la valeur de l'int est moins grande.)")
    return seed
